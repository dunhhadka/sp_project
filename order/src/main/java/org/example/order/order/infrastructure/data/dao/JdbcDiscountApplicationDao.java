...
public class JdbcDiscountApplicationDao implements DiscountApplicationDao {
    ...
    @Override
    public List<DiscountApplicationDto> getByOrderIds(int storeId, List<Integer> orderIds) {
        if (CollectionUtils.isEmpty(orderIds)) return List.of();
        return jdbcTemplate.query(
                "SELECT * FROM discount_applications WHERE store_id = :storeId AND order_id IN (:orderIds)",
